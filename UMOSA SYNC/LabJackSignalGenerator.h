...
public:
    ...
    //send message that is returned from messageGetter with length 'length'. The message is sampled at the moment the first symbol is starting to sent
    void sendMessage(uint64_t(*messageGetter)(), uint8_t length);
